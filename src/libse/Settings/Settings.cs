...
namespace Nikse.SubtitleEdit.Core.Settings
{
    ...
    public class Settings
    {
        ...
        public static Settings CustomDeserialize(string fileName)
        {
            ...
            subNode = node.SelectSingleNode("SplitAtSentenceBoundary");
            if (subNode != null)
            {
                settings.General.SplitAtSentenceBoundary = Convert.ToBoolean(subNode.InnerText.Trim());
            }

            subNode = node.SelectSingleNode("OriginalPairByStyle");
            if (subNode != null)
            {
